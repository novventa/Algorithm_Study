 ...
 * 1-1. 현재 조합에서 사용할 행을 표시하는 injectioned 배열을 만들자.
...
public class SWEA_2112_보호필름_변지혜 {
	...
	private static void comb(int start, int depth, int maxDepth) {
		...
		// 기저조건
		if (depth == maxDepth) {
			// 현재 조합에서 사용할 행의 개수만큼 다 골랐다면
			// 사용할 행에 A 또는 B 약품을 주입해보자
			inject(0, maxDepth);
			return;
		}
		...
		for (int idx = start; idx < rowSize; idx++) {
			injectioned[depth] = idx;
			comb(idx + 1, depth + 1, maxDepth); // 다음 깊이를 확인하러 가자
		}
		
	}
	
	private static void inject(int depth, int maxDepth) {
		// 각 행에 약품을 주입하는 메서드
		...
		// 기저 조건
		if (depth == maxDepth) {
			// 모든 행에 주입할 약품을 다 골랐다면
			// 약품을 다 주입하고 합격 기준을 만족하는지 확인해보자
			isPossible();
		...
		for (int col = 0; col < colSize; col++)
			film[row][col] = 0;
		inject(depth + 1, maxDepth); // 다음 행에 약품을 주입시키자
	...
	public static void main(String[] args) throws NumberFormatException, IOException {
		...
		for (int tc = 1; tc <= testCase; tc++) { // 테스트케이스 개수만큼 반복
			...
			// stringTokenizer가 String배열보다 메모리를 덜 잡아먹나 테스트해봤으나
			// 도찐개찐.. 별 차이가 없음
			// 실행시간에도 별 차이 없음
//			StringTokenizer st = new StringTokenizer(br.readLine());
			...
			film = new int[rowSize][colSize]; // D*W 크기의 필름 배열
...
//				st = new StringTokenizer(br.readLine());
				...
				for (int col = 0; col < colSize; col++) {
					film[row][col] = Integer.parseInt(line[col]);
				}
			}
			...
			for (int cnt = 0; cnt < rowSize && !isMeet; cnt++) {
				injectioned = new int[cnt + 1]; // 현재 조합에 사용된 행 정보를 저장할 배열
