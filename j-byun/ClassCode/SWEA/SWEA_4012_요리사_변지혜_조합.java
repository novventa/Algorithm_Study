...
public class SWEA_4012_요리사_변지혜_조합 {
	...
	private static void comb(int start, int depth, int maxDepth) {
		...
		for (int idx = start; idx < foodCnt; idx++) {
			if(used[idx]) continue; // 이미 사용한 재료라면 다음 재료 확인
			used[idx] = true; // 아직 사용하지 않은 재료라면 사용하자
			comb(idx + 1, depth + 1, maxDepth); // 다음 재료 확인하러 가자
			used[idx] = false; // 사용 끝났으면 미사용 상태로 돌려주자
		}
	}
	...
	private static void checkSynergy(int start, int depth, int maxDepth) {
		...
		// 두 개의 조합 만들기
		for (int idx = start; idx < foodCnt / 2; idx++) {
			if (used2[idx]) continue;
			used2[idx] = true;
			checkSynergy(idx + 1, depth + 1, maxDepth);
			used2[idx] = false;
		}
