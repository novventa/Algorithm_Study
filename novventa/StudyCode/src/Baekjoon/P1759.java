...
public class P1759 {
    ...
    // 기본적으로 브루트포스 알고리즘
        ...
        // 입력받은 알파벳들을 저장할 배열
        static char[] alpha;
        // 스트링빌더로 모든 가능성있는 암호를 저장한 문자열을 만들기 위해 선언
        ...
        public static void combination(int idx, int vowel, int cons, int count, StringBuilder password) {
            ...
            // 모든 가능한 조합을 모두 만들어본다.
            // 따라서 문자를 선택한 경우, 선택 안한 경우
            // 두 가지 모두 해본 뒤 위 조건식에서 걸러지는 문자열만 sb에 저장한다.

            // 현재 인덱스의 문자를 선택하는 경우
            // 파라미터로 받은 문자들이 저장된 스트링빌더를 바탕으로 문자를 이어 저장하기 위해 updatedPassword를 password 객체를 복사하여 생성한다.
            ...
            // 선택한 문자가 자음이면 자음의 갯수 1 증가 아니면 0 증가, 문자를 하나 추가했으므로 카운트도 1 증가
